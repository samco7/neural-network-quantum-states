...
class NQS(object):
    ...
    def __init__(self, n_visible, n_hidden, a=None, b=None, W=None, init_weight=0.001):
        """
        ...
        if b:
            ...
            self.b = init_weight * (np.random.rand(n_hidden, 1) - 0.5)
        if W:
            self.W = W
        else:
    ...
    # def update_params_cd(self, da, db, dW):
    #     self.a -= da
    #     self.b -= db
    #     self.W -= dW
    #     self.thetas = self.b
    ...
    def amplitude_ratio(self, state, flips_to_new_state):
        ...
        state to state
        """
        ratio = np.exp(self.log_amplitude_ratio(state, flips_to_new_state))
        return ratio
